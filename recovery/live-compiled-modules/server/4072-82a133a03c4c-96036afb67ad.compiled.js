...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/orders/[orderNumber]/payment-proof/route.ts
// Project-relative source path: app/api/orders/[orderNumber]/payment-proof/route.ts
// Module ID: 4072
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 96036afb67adf94208973949de4aa743b8892cd300937fdf19dd98574d41d474
...
((a2, b4, c2) => {
          ...
          var e = c2(95736), f = c2(9117), g2 = c2(4044), h = c2(39326), i = c2(32324), j3 = c2(261), k3 = c2(54290), l = c2(85328), m2 = c2(38928), n = c2(46595), o = c2(3421), p2 = c2(17679), q3 = c2(41681), r = c2(63446), s = c2(86439), t = c2(51356), u = c2(10641), v2 = c2(48689), w3 = c2(91132);
          let x3 = /* @__PURE__ */ new Set(["image/jpeg", "image/png", "image/webp", "image/heic", "image/heif"]), y3 = /* @__PURE__ */ new Set(["jpg", "jpeg", "png", "webp", "heic", "heif"]);
          async function z3(a3, { params: b5 }) {
            let { orderNumber: c3 } = await b5, d2 = (0, w3.qK)(), e2 = (0, w3.nk)();
            if (!d2 || !e2) return u.NextResponse.json({ success: false, error: "Upload service unavailable" }, { status: 503 });
            let f2 = (await a3.formData()).get("file");
            if (!f2) return u.NextResponse.json({ success: false, error: "No screenshot selected" }, { status: 400 });
            let g3 = (function(a4) {
              let b6 = a4.name.split(".").pop()?.toLowerCase();
              return b6 && y3.has(b6) ? b6 : a4.type === "image/jpeg" ? "jpg" : a4.type === "image/png" ? "png" : a4.type === "image/webp" ? "webp" : a4.type === "image/heic" ? "heic" : a4.type === "image/heif" ? "heif" : null;
            })(f2);
            if (!g3 || f2.type && !x3.has(f2.type)) return u.NextResponse.json({ success: false, error: "Please upload a screenshot image" }, { status: 400 });
            if (f2.size > 5242880) return u.NextResponse.json({ success: false, error: "Screenshot must be under 5MB" }, { status: 400 });
            let h2 = (0, w3.Lf)(d2), i2 = await h2.select().from(w3.Ww).where((0, v2.eq)(w3.Ww.orderNumber, c3)).get();
            if (!i2) return u.NextResponse.json({ success: false, error: "Order not found" }, { status: 404 });
            if (i2.paymentMethod === "cod") return u.NextResponse.json({ success: false, error: "Payment proof is not needed for COD orders" }, { status: 400 });
            let j4 = `payment-proofs/${i2.id}/${(0, w3.$C)("proof")}.${g3}`;
            await e2.put(j4, await f2.arrayBuffer(), { httpMetadata: { contentType: f2.type && x3.has(f2.type) ? f2.type : g3 === "jpg" || g3 === "jpeg" ? "image/jpeg" : g3 === "png" ? "image/png" : g3 === "webp" ? "image/webp" : g3 === "heic" ? "image/heic" : g3 === "heif" ? "image/heif" : "application/octet-stream" } });
            let k4 = (/* @__PURE__ */ new Date()).toISOString();
            return await h2.transaction(async (a4) => {
              await a4.update(w3.Ww).set({ paymentScreenshotUrl: j4, updatedAt: k4 }).where((0, v2.eq)(w3.Ww.id, i2.id)), await a4.insert(w3.Y4).values({ id: (0, w3.$C)("act"), orderId: i2.id, action: w3.LM.NOTE_ADDED, details: JSON.stringify({ note: "Payment proof uploaded by customer", filename: f2.name, size: f2.size, mimeType: f2.type || null }), performedBy: "customer" });
            }), u.NextResponse.json({ success: true });
          }
          __name(z3, "z3");
          let A2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/orders/[orderNumber]/payment-proof/route", pathname: "/api/orders/[orderNumber]/payment-proof", filename: "route", bundlePath: "app/api/orders/[orderNumber]/payment-proof/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/orders/[orderNumber]/payment-proof/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: B2, workUnitAsyncStorage: C2, serverHooks: D2 } = A2;
          function E2() {
          ...
          async function F2(a3, b5, c3) {
            var d2;
            let e2 = "/api/orders/[orderNumber]/payment-proof/route";
            e2 === "/index" && (e2 = "/");
