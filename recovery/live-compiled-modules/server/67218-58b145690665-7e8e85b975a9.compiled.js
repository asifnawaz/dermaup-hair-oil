...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/track/route.ts
// Project-relative source path: app/api/track/route.ts
// Module ID: 67218
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 7e8e85b975a9a02bc0e94671054a92bdda8f035bfda7ec31fccbffc2042f8563
...
((a2, b4, c2) => {
          ...
          async function w3(a3) {
            try {
              let b5, c3 = await a3.text();
              if (!c3.trim()) return u.NextResponse.json({ success: false, error: "request body is required" }, { status: 400 });
              try {
                b5 = JSON.parse(c3);
              } catch {
                return u.NextResponse.json({ success: false, error: "invalid JSON body" }, { status: 400 });
              }
              let { event: d2, properties: e2 } = b5;
              if (typeof d2 != "string" || !d2.trim()) return u.NextResponse.json({ success: false, error: "event is required" }, { status: 400 });
              let f2 = (function(a4) {
                return a4.cookies.get("upderma_session")?.value || `ses_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
              })(a3), g3 = (0, v2.qK)();
              if (!g3) {
                console.log("[Track Event]", d2, e2, f2);
                let b6 = u.NextResponse.json({ success: true, stored: false });
                return a3.cookies.get("upderma_session") || b6.cookies.set("upderma_session", f2, { maxAge: 604800, httpOnly: true, secure: true, sameSite: "lax" }), b6;
              }
              let h2 = (0, v2.Lf)(g3), i2 = a3.headers.get("user-agent") || void 0, j4 = a3.headers.get("cf-ipcountry") || void 0;
              await h2.insert(v2.sK).values({ id: (0, v2.$C)("evt"), eventType: d2.trim(), eventData: e2 ? JSON.stringify(e2) : null, sessionId: f2, userAgent: i2, ipCountry: j4 });
              let k4 = u.NextResponse.json({ success: true, stored: true });
              return a3.cookies.get("upderma_session") || k4.cookies.set("upderma_session", f2, { maxAge: 604800, httpOnly: true, secure: true, sameSite: "lax" }), k4;
            } catch (a4) {
              return console.error("Track event error:", a4), u.NextResponse.json({ success: false, error: "Failed to store event" }, { status: 500 });
            }
          }
          __name(w3, "w3");
          let x3 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/track/route", pathname: "/api/track", filename: "route", bundlePath: "app/api/track/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/track/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: y3, workUnitAsyncStorage: z3, serverHooks: A2 } = x3;
          function B2() {
