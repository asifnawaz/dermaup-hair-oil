...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/content/blocks/route.ts
// Project-relative source path: app/api/admin/content/blocks/route.ts
// Module ID: 71714
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: bd5fdc0d41f1cdbd9d3ff8ce36f0c26bde38a43a74ef128388abf98c3b100530
...
((a2, b4, c2) => {
          ...
          async function B2(a3) {
            ...
            let { searchParams: c3 } = new URL(a3.url), d2 = c3.get("type"), e2 = c3.get("productId"), f2 = (0, y3.qK)();
            if (!f2) return u.NextResponse.json({ success: false, error: "DB not available" }, { status: 500 });
            let g3 = (0, y3.Lf)(f2), h2 = [];
            d2 && h2.push((0, w3.eq)(y3.Id.type, d2)), e2 && h2.push((0, w3.eq)(y3.Id.productId, e2));
            let i2 = h2.length > 0 ? await g3.select().from(y3.Id).where((0, w3.Uo)(...h2)).orderBy((0, x3.Y)(y3.Id.sortOrder)).all() : await g3.select().from(y3.Id).orderBy((0, x3.Y)(y3.Id.type), (0, x3.Y)(y3.Id.sortOrder)).all();
            return u.NextResponse.json({ success: true, data: i2.map((a4) => ({ ...a4, parsedData: (0, A2.$)(a4.data) })) });
          }
          __name(B2, "B2");
          async function C2(a3) {
            ...
            let d2 = (0, y3.Lf)(c3), e2 = await a3.json(), f2 = (0, y3.$C)("cb");
            return await d2.insert(y3.Id).values({ id: f2, type: e2.type, slug: e2.slug, productId: e2.productId || null, data: JSON.stringify(e2.data), active: e2.active ?? true, sortOrder: e2.sortOrder ?? 0 }), (0, v2.revalidateTag)(`blocks:${e2.type}`), e2.productId && (0, v2.revalidateTag)(`all-blocks:${e2.productId}`), u.NextResponse.json({ success: true, data: { id: f2 } }, { status: 201 });
          }
          __name(C2, "C2");
          let D2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/content/blocks/route", pathname: "/api/admin/content/blocks", filename: "route", bundlePath: "app/api/admin/content/blocks/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/content/blocks/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: E2, workUnitAsyncStorage: F2, serverHooks: G3 } = D2;
          function H3() {
          ...
          async function I3(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/content/blocks/route";
            e2 === "/index" && (e2 = "/");
