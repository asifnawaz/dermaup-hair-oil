...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/users/[id]/route.ts
// Project-relative source path: app/api/admin/users/[id]/route.ts
// Module ID: 18752
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 53ce806a3ba35ebc39c0e099a1f573305e4f581afc0966b22ab7ec402ce46c91
...
((a2, b4, c2) => {
          ...
          var d = {};
          c2.r(d), c2.d(d, { DELETE: /* @__PURE__ */ __name(() => y3, "DELETE") });
          ...
          async function y3(a3, { params: b5 }) {
            try {
              ...
              if (!(0, x3.oC)(c3)) return u.NextResponse.json({ success: false, error: "Unauthorized" }, { status: 401 });
              if (!(0, x3.Kv)(c3)) return u.NextResponse.json({ success: false, error: "Super admin access required" }, { status: 403 });
              let { id: d2 } = await b5;
              if (c3.id === d2) return u.NextResponse.json({ success: false, error: "Cannot delete your own account" }, { status: 400 });
              let e2 = (0, w3.qK)();
              if (!e2) return u.NextResponse.json({ success: false, error: "Database not available" }, { status: 500 });
              let f2 = (0, w3.Lf)(e2);
              return await f2.select().from(w3.qM).where((0, v2.eq)(w3.qM.id, d2)).get() ? (await f2.delete(w3.qM).where((0, v2.eq)(w3.qM.id, d2)), u.NextResponse.json({ success: true, message: "Admin user deleted successfully" })) : u.NextResponse.json({ success: false, error: "User not found" }, { status: 404 });
            } catch (a4) {
              return console.error("Delete admin error:", a4), u.NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
            }
          }
          __name(y3, "y3");
          let z3 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/users/[id]/route", pathname: "/api/admin/users/[id]", filename: "route", bundlePath: "app/api/admin/users/[id]/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/users/[id]/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: A2, workUnitAsyncStorage: B2, serverHooks: C2 } = z3;
          function D2() {
          ...
          async function E2(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/users/[id]/route";
            e2 === "/index" && (e2 = "/");
