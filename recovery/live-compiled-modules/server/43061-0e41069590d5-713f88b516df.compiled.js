...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/content/pages/route.ts
// Project-relative source path: app/api/admin/content/pages/route.ts
// Module ID: 43061
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 713f88b516dfb5ed16ab780e20d698fadc2a0e96fd6268141182826b41b9cf2f
...
((a2, b4, c2) => {
          ...
          async function B2(a3) {
            ...
            let { searchParams: c3 } = new URL(a3.url), d2 = c3.get("productId"), e2 = (0, y3.qK)();
            if (!e2) return u.NextResponse.json({ success: false, error: "DB not available" }, { status: 500 });
            let f2 = (0, y3.Lf)(e2), g3 = d2 ? f2.select().from(y3.Bd).where((0, w3.eq)(y3.Bd.productId, d2)).orderBy((0, x3.Y)(y3.Bd.slug)) : f2.select().from(y3.Bd).orderBy((0, x3.Y)(y3.Bd.slug)), h2 = await g3.all();
            return u.NextResponse.json({ success: true, data: h2.map((a4) => ({ ...a4, parsedMeta: (0, A2.$)(a4.meta) })) });
          }
          __name(B2, "B2");
          async function C2(a3) {
            ...
            let d2 = (0, y3.Lf)(c3), e2 = await a3.json(), f2 = (0, y3.$C)("page");
            await d2.insert(y3.Bd).values({ id: f2, slug: e2.slug, productId: e2.productId || null, title: e2.title, meta: e2.meta ? JSON.stringify(e2.meta) : null, active: e2.active ?? true });
            let g3 = e2.template ? { "product-landing": ["hero", "problem", "solution", "ingredients", "before_after", "testimonials", "benefits", "pricing", "faq", "checkout", "cta"], "sale-campaign": ["promo_banner", "hero", "pricing", "testimonials", "social_proof_bar", "faq", "checkout", "cta"], "content-page": ["hero", "education", "benefits", "how_it_works", "cta"] }[e2.template] : null;
            if (g3) for (let a4 = 0; a4 < g3.length; a4++) await d2.insert(y3.Kz).values({ id: (0, y3.$C)("ps"), pageId: f2, sectionType: g3[a4], sortOrder: a4, active: true });
            return (0, v2.revalidateTag)(`page:${e2.slug}`), u.NextResponse.json({ success: true, data: { id: f2 } }, { status: 201 });
          }
          __name(C2, "C2");
          let D2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/content/pages/route", pathname: "/api/admin/content/pages", filename: "route", bundlePath: "app/api/admin/content/pages/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/content/pages/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: E2, workUnitAsyncStorage: F2, serverHooks: G3 } = D2;
          function H3() {
          ...
          async function I3(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/content/pages/route";
            e2 === "/index" && (e2 = "/");
