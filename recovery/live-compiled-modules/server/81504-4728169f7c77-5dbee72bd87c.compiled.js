...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/media/route.ts
// Project-relative source path: app/api/admin/media/route.ts
// Module ID: 81504
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 5dbee72bd87cfaef606a6a0491473390ab36842b35ac0bcd88dd6d3b4149ae68
// The factory below is preserved as data and is not executed by the extractor.

((a2, b4, c2) => {
          "use strict";
          c2.r(b4), c2.d(b4, { handler: /* @__PURE__ */ __name(() => H3, "handler"), patchFetch: /* @__PURE__ */ __name(() => G3, "patchFetch"), routeModule: /* @__PURE__ */ __name(() => C2, "routeModule"), serverHooks: /* @__PURE__ */ __name(() => F2, "serverHooks"), workAsyncStorage: /* @__PURE__ */ __name(() => D2, "workAsyncStorage"), workUnitAsyncStorage: /* @__PURE__ */ __name(() => E2, "workUnitAsyncStorage") });
          var d = {};
          c2.r(d), c2.d(d, { GET: /* @__PURE__ */ __name(() => A2, "GET"), POST: /* @__PURE__ */ __name(() => B2, "POST") });
          ...
          let z3 = ["image/jpeg", "image/png", "image/webp", "image/gif", "image/svg+xml"];
          async function A2(a3) {
            ...
            let { searchParams: c3 } = new URL(a3.url), d2 = c3.get("productId"), e2 = (0, x3.qK)();
            if (!e2) return u.NextResponse.json({ success: false, error: "DB not available" }, { status: 500 });
            let f2 = (0, x3.Lf)(e2), g3 = d2 ? f2.select().from(x3.$_).where((0, v2.eq)(x3.$_.productId, d2)).orderBy((0, w3.i)(x3.$_.createdAt)) : f2.select().from(x3.$_).orderBy((0, w3.i)(x3.$_.createdAt)), h2 = await g3.all();
            return u.NextResponse.json({ success: true, data: h2 });
          }
          __name(A2, "A2");
          async function B2(a3) {
            ...
            if (!(0, y3.oC)(b5)) return u.NextResponse.json({ success: false, error: "Not authenticated" }, { status: 401 });
            let c3 = (0, x3.qK)();
            if (!c3) return u.NextResponse.json({ success: false, error: "DB not available" }, { status: 500 });
            let d2 = await a3.formData(), e2 = d2.get("file"), f2 = d2.get("productId"), g3 = d2.get("alt");
            if (!e2) return u.NextResponse.json({ success: false, error: "No file provided" }, { status: 400 });
            if (!z3.includes(e2.type)) return u.NextResponse.json({ success: false, error: `File type ${e2.type} not allowed` }, { status: 400 });
            if (e2.size > 5242880) return u.NextResponse.json({ success: false, error: "File too large (max 5MB)" }, { status: 400 });
            let h2 = (0, x3.$C)("med"), i2 = e2.name.split(".").pop() || "bin", j4 = `media/${h2}.${i2}`, k4 = await e2.arrayBuffer(), l2 = (0, x3.nk)();
            return l2 ? (await l2.put(j4, k4, { httpMetadata: { contentType: e2.type } }), await (0, x3.Lf)(c3).insert(x3.$_).values({ id: h2, key: j4, filename: e2.name, alt: g3 || null, mimeType: e2.type, size: e2.size, productId: f2 || null }), u.NextResponse.json({ success: true, data: { id: h2, key: j4, filename: e2.name, mimeType: e2.type, size: e2.size, url: `/api/admin/media/${h2}/file` } }, { status: 201 })) : u.NextResponse.json({ success: false, error: "Media bucket not available" }, { status: 500 });
          }
          __name(B2, "B2");
          let C2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/media/route", pathname: "/api/admin/media", filename: "route", bundlePath: "app/api/admin/media/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/media/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: D2, workUnitAsyncStorage: E2, serverHooks: F2 } = C2;
          function G3() {
            return (0, g2.patchFetch)({ workAsyncStorage: D2, workUnitAsyncStorage: E2 });
          }
          __name(G3, "G3");
          async function H3(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/media/route";
            e2 === "/index" && (e2 = "/");
            let g3 = await C2.prepare(a3, b5, { srcPage: e2, multiZoneDraftMode: false });
            if (!g3) return b5.statusCode = 400, b5.end("Bad Request"), c3.waitUntil == null || c3.waitUntil.call(c3, Promise.resolve()), null;
            let { buildId: u2, params: v3, nextConfig: w4, isDraftMode: x4, prerenderManifest: y4, routerServerContext: z4, isOnDemandRevalidate: A3, revalidateOnlyGenerated: B3, resolvedPathname: D3 } = g3, E3 = (0, j3.normalizeAppPath)(e2), F3 = !!(y4.dynamicRoutes[E3] || y4.routes[D3]);
            if (F3 && !x4) {
              let a4 = !!y4.routes[D3], b6 = y4.dynamicRoutes[E3];
              if (b6 && b6.fallback === false && !a4) throw new s.NoFallbackError();
            }
            let G4 = null;
            !F3 || C2.isDev || x4 || (G4 = (G4 = D3) === "/index" ? "/" : G4);
            let H4 = C2.isDev === true || !F3, I3 = F3 && !H4, J2 = a3.method || "GET", K3 = (0, i.getTracer)(), L3 = K3.getActiveScopeSpan(), M3 = { params: v3, prerenderManifest: y4, renderOpts: { experimental: { cacheComponents: !!w4.experimental.cacheComponents, authInterrupts: !!w4.experimental.authInterrupts }, supportsDynamicResponse: H4, incrementalCache: (0, h.getRequestMeta)(a3, "incrementalCache"), cacheLifeProfiles: (d2 = w4.experimental) == null ? void 0 : d2.cacheLife, isRevalidate: I3, waitUntil: c3.waitUntil, onClose: /* @__PURE__ */ __name((a4) => {
              b5.on("close", a4);
            }, "onClose"), onAfterTaskError: void 0, onInstrumentationRequestError: /* @__PURE__ */ __name((b6, c4, d3) => C2.onRequestError(a3, b6, d3, z4), "onInstrumentationRequestError") }, sharedContext: { buildId: u2 } }, N3 = new k3.NodeNextRequest(a3), O3 = new k3.NodeNextResponse(b5), P2 = l.NextRequestAdapter.fromNodeNextRequest(N3, (0, l.signalFromNodeResponse)(b5));
            try {
              let d3 = /* @__PURE__ */ __name(async (c4) => C2.handle(P2, M3).finally(() => {
                if (!c4) return;
                ...
                let k4 = /* @__PURE__ */ __name(async ({ previousCacheEntry: f2 }) => {
                  try {
                    ...
                    throw f2?.isStale && await C2.onRequestError(a3, b6, { routerKind: "App Router", routePath: e2, routeType: "route", revalidateReason: (0, n.c)({ isRevalidate: I3, isOnDemandRevalidate: A3 }) }, z4), b6;
                  }
                }, "k4"), l2 = await C2.handleResponse({ req: a3, nextConfig: w4, cacheKey: G4, routeKind: f.RouteKind.APP_ROUTE, isFallback: false, prerenderManifest: y4, isRoutePPREnabled: false, isOnDemandRevalidate: A3, revalidateOnlyGenerated: B3, responseGenerator: k4, waitUntil: c3.waitUntil });
                if (!F3) return null;
              ...
              if (b6 instanceof s.NoFallbackError || await C2.onRequestError(a3, b6, { routerKind: "App Router", routePath: E3, routeType: "route", revalidateReason: (0, n.c)({ isRevalidate: I3, isOnDemandRevalidate: A3 }) }), F3) throw b6;
