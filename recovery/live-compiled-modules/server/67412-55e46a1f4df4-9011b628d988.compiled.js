...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/orders/route.ts
// Project-relative source path: app/api/admin/orders/route.ts
// Module ID: 67412
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 9011b628d988ce9ab80d4f3f844f79fe22060bfe37abd655d32b35a569d44ad8
...
((a2, b4, c2) => {
          ...
          var e = c2(95736), f = c2(9117), g2 = c2(4044), h = c2(39326), i = c2(32324), j3 = c2(261), k3 = c2(54290), l = c2(85328), m2 = c2(38928), n = c2(46595), o = c2(3421), p2 = c2(17679), q3 = c2(41681), r = c2(63446), s = c2(86439), t = c2(51356), u = c2(10641), v2 = c2(85766), w3 = c2(48689), x3 = c2(71559), y3 = c2(91132), z3 = c2(66147);
          async function A2(a3) {
            ...
            try {
              let { searchParams: b6 } = new URL(a3.url), c3 = parseInt(b6.get("page") || "1"), d2 = parseInt(b6.get("limit") || "20"), e2 = b6.get("status"), f2 = b6.get("paymentStatus"), g3 = b6.get("search"), h2 = b6.get("city"), i2 = (0, y3.qK)();
              if (!i2) return u.NextResponse.json({ success: true, data: { orders: [], pagination: { page: c3, limit: d2, total: 0, totalPages: 0 } } });
              let j4 = (0, y3.Lf)(i2), k4 = [];
              e2 && k4.push((0, v2.ll)`${y3.Ww.orderStatus} = ${e2}`), f2 && k4.push((0, v2.ll)`${y3.Ww.paymentStatus} = ${f2}`), h2 && k4.push((0, w3.eq)(y3.Ww.city, h2)), g3 && k4.push((0, v2.ll)`(${y3.Ww.orderNumber} LIKE ${"%" + g3 + "%"} OR ${y3.Ww.customerName} LIKE ${"%" + g3 + "%"} OR ${y3.Ww.customerPhone} LIKE ${"%" + g3 + "%"})`);
              let l2 = await j4.select({ count: (0, v2.ll)`count(*)` }).from(y3.Ww).where(k4.length > 0 ? (0, w3.Uo)(...k4) : void 0).get(), m3 = l2?.count || 0, n2 = (c3 - 1) * d2, o2 = await j4.select().from(y3.Ww).where(k4.length > 0 ? (0, w3.Uo)(...k4) : void 0).orderBy((0, x3.i)(y3.Ww.createdAt)).limit(d2).offset(n2).all();
              return u.NextResponse.json({ success: true, data: { orders: o2, pagination: { page: c3, limit: d2, total: m3, totalPages: Math.ceil(m3 / d2) } } });
            } catch (a4) {
              return console.error("Get orders error:", a4), u.NextResponse.json({ success: false, error: "Failed to fetch orders" }, { status: 500 });
            }
          }
          __name(A2, "A2");
          let B2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/orders/route", pathname: "/api/admin/orders", filename: "route", bundlePath: "app/api/admin/orders/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/orders/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: C2, workUnitAsyncStorage: D2, serverHooks: E2 } = B2;
          function F2() {
