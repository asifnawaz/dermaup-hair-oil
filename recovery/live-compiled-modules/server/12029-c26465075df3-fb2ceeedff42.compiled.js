...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/customers/route.ts
// Project-relative source path: app/api/admin/customers/route.ts
// Module ID: 12029
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: fb2ceeedff42580e7a6cd64348fc270e5014e6d603495223e4a9a91937aa2a99
...
((a2, b4, c2) => {
          ...
          var e = c2(95736), f = c2(9117), g2 = c2(4044), h = c2(39326), i = c2(32324), j3 = c2(261), k3 = c2(54290), l = c2(85328), m2 = c2(38928), n = c2(46595), o = c2(3421), p2 = c2(17679), q3 = c2(41681), r = c2(63446), s = c2(86439), t = c2(51356), u = c2(10641), v2 = c2(91132), w3 = c2(66147);
          async function x3(a3) {
            let b5 = await (0, w3.K5)(a3);
            if (!(0, w3.oC)(b5)) return u.NextResponse.json({ success: false, error: "Not authenticated" }, { status: 401 });
            try {
              let { searchParams: b6 } = new URL(a3.url), c3 = parseInt(b6.get("page") || "1"), d2 = parseInt(b6.get("limit") || "20"), e2 = b6.get("search") || "", f2 = (c3 - 1) * d2, g3 = (0, v2.qK)();
              if (!g3) return u.NextResponse.json({ success: true, data: { customers: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } } });
              (0, v2.Lf)(g3);
              let h2 = "";
              if (e2) {
                let a4 = e2.replace(/'/g, "''");
                h2 = `WHERE customer_phone LIKE '%${a4}%' OR customer_name LIKE '%${a4}%' OR customer_email LIKE '%${a4}%'`;
              }
              let i2 = await g3.prepare(`SELECT COUNT(*) as total FROM (SELECT customer_phone FROM orders ${h2} GROUP BY customer_phone)`).first(), j4 = i2?.total || 0, k4 = await g3.prepare(`SELECT
        ...
        LIMIT ? OFFSET ?`).bind(d2, f2).all();
              return u.NextResponse.json({ success: true, data: { customers: k4.results || [], pagination: { page: c3, limit: d2, total: j4, totalPages: Math.ceil(j4 / d2) } } });
            } catch (a4) {
              return console.error("Customers list error:", a4), u.NextResponse.json({ success: false, error: "Failed to fetch customers" }, { status: 500 });
            }
          }
          __name(x3, "x3");
          let y3 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/customers/route", pathname: "/api/admin/customers", filename: "route", bundlePath: "app/api/admin/customers/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/customers/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: z3, workUnitAsyncStorage: A2, serverHooks: B2 } = y3;
          function C2() {
          ...
          async function D2(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/customers/route";
            e2 === "/index" && (e2 = "/");
