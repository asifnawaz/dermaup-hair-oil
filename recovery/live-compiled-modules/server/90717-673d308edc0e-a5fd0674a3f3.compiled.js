...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/coupons/route.ts
// Project-relative source path: app/api/admin/coupons/route.ts
// Module ID: 90717
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: a5fd0674a3f3e93473479cec697cfef8f9d13e4af15f7592cd8e780e6d4474a8
...
((a2, b4, c2) => {
          ...
          var e = c2(95736), f = c2(9117), g2 = c2(4044), h = c2(39326), i = c2(32324), j3 = c2(261), k3 = c2(54290), l = c2(85328), m2 = c2(38928), n = c2(46595), o = c2(3421), p2 = c2(17679), q3 = c2(41681), r = c2(63446), s = c2(86439), t = c2(51356), u = c2(10641), v2 = c2(48689), w3 = c2(69334), x3 = c2(71559), y3 = c2(91132), z3 = c2(66147), A2 = c2(5989);
          async function B2(a3) {
            ...
            try {
              let { searchParams: b6 } = new URL(a3.url), c3 = parseInt(b6.get("page") || "1"), d2 = parseInt(b6.get("limit") || "20"), e2 = b6.get("search") || "", f2 = (c3 - 1) * d2, g3 = (0, y3.qK)();
              if (!g3) return u.NextResponse.json({ success: true, data: { coupons: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } } });
              let h2 = (0, y3.Lf)(g3), i2 = e2 ? (0, v2.mj)(y3.aC.code, `%${e2}%`) : void 0, j4 = await h2.select({ count: (0, w3.U9)() }).from(y3.aC).where(i2).get(), k4 = j4?.count || 0, l2 = await h2.select().from(y3.aC).where(i2).orderBy((0, x3.i)(y3.aC.createdAt)).limit(d2).offset(f2).all();
              return u.NextResponse.json({ success: true, data: { coupons: l2, pagination: { page: c3, limit: d2, total: k4, totalPages: Math.ceil(k4 / d2) } } });
            } catch (a4) {
              return a4?.message?.includes("no such table") || a4?.cause?.message?.includes("no such table") ? u.NextResponse.json({ success: true, data: { coupons: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } } }) : (console.error("Coupons list error:", a4), u.NextResponse.json({ success: false, error: "Failed to fetch coupons" }, { status: 500 }));
            }
          }
          __name(B2, "B2");
          async function C2(a3) {
            ...
            if (!(0, z3.oC)(b5)) return u.NextResponse.json({ success: false, error: "Not authenticated" }, { status: 401 });
            try {
              let b6 = await a3.json(), c3 = A2.Cm.safeParse(b6);
              if (!c3.success) return u.NextResponse.json({ success: false, error: c3.error.errors[0]?.message || "Validation failed" }, { status: 400 });
              let d2 = (0, y3.qK)();
              if (!d2) return u.NextResponse.json({ success: false, error: "DB unavailable" }, { status: 503 });
              let e2 = (0, y3.Lf)(d2), f2 = c3.data;
              if (await e2.select().from(y3.aC).where((0, v2.eq)(y3.aC.code, f2.code.toUpperCase())).get()) return u.NextResponse.json({ success: false, error: "Coupon code already exists" }, { status: 409 });
              let g3 = (0, y3.$C)("cpn");
              return await e2.insert(y3.aC).values({ id: g3, code: f2.code.toUpperCase(), type: f2.type, value: f2.value, minOrder: f2.minOrder, maxUses: f2.maxUses, appliesTo: f2.appliesTo, active: f2.active, startsAt: f2.startsAt || null, expiresAt: f2.expiresAt || null }), u.NextResponse.json({ success: true, data: { id: g3 } }, { status: 201 });
            } catch (a4) {
              return console.error("Coupon create error:", a4), u.NextResponse.json({ success: false, error: "Failed to create coupon" }, { status: 500 });
            }
          }
          __name(C2, "C2");
          let D2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/coupons/route", pathname: "/api/admin/coupons", filename: "route", bundlePath: "app/api/admin/coupons/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/coupons/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: E2, workUnitAsyncStorage: F2, serverHooks: G3 } = D2;
          function H3() {
          ...
          async function I3(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/coupons/route";
            e2 === "/index" && (e2 = "/");
