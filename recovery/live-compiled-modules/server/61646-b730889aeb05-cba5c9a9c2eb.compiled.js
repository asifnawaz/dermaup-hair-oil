...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/setup/route.ts
// Project-relative source path: app/api/admin/setup/route.ts
// Module ID: 61646
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: cba5c9a9c2ebeda7d97ab32fe0eda277a90b2527fe8075c813292ae9bdf557df
...
((a2, b4, c2) => {
          ...
          var d = {};
          c2.r(d), c2.d(d, { GET: /* @__PURE__ */ __name(() => D2, "GET"), POST: /* @__PURE__ */ __name(() => C2, "POST") });
          var e = c2(95736), f = c2(9117), g2 = c2(4044), h = c2(39326), i = c2(32324), j3 = c2(261), k3 = c2(54290), l = c2(85328), m2 = c2(38928), n = c2(46595), o = c2(3421), p2 = c2(17679), q3 = c2(41681), r = c2(63446), s = c2(86439), t = c2(51356), u = c2(10641), v2 = c2(2995), w3 = c2(35559), x3 = c2(91132), y3 = c2(66147), z3 = c2(55386), A2 = c2(48689);
          let B2 = v2.Ik({ email: v2.Yj().email(), password: v2.Yj().min(6), name: v2.Yj().min(2), setupKey: v2.Yj().min(1) });
          async function C2(a3) {
            try {
              let b5 = await a3.json(), { email: c3, password: d2, name: e2, setupKey: f2 } = B2.parse(b5), g3 = (0, z3.rv)("ADMIN_SETUP_KEY");
              if (f2 !== g3) return u.NextResponse.json({ success: false, error: "Invalid setup key" }, { status: 403 });
              let h2 = (0, x3.qK)();
              if (!h2) return u.NextResponse.json({ success: false, error: "Database not available" }, { status: 500 });
              let i2 = (0, x3.Lf)(h2);
              if ((await i2.select().from(x3.W0).where((0, A2.eq)(x3.W0.key, "setup_complete")).get())?.value === "true") return u.NextResponse.json({ success: false, error: "Setup already completed. Use admin panel to add more users." }, { status: 400 });
              if (await i2.select().from(x3.qM).get()) return await i2.insert(x3.W0).values({ key: "setup_complete", value: "true" }).onConflictDoUpdate({ target: x3.W0.key, set: { value: "true" } }), u.NextResponse.json({ success: false, error: "Admin already exists. Use admin panel to add more users." }, { status: 400 });
              let k4 = await (0, y3.Er)(d2), l2 = (0, x3.$C)("admin");
              return await i2.insert(x3.qM).values({ id: l2, email: c3, passwordHash: k4, name: e2, role: "super_admin" }), await i2.insert(x3.W0).values({ key: "setup_complete", value: "true" }), u.NextResponse.json({ success: true, message: "Admin account created successfully", data: { id: l2, email: c3, name: e2, role: "super_admin" } });
            } catch (a4) {
              return a4 instanceof w3.G ? u.NextResponse.json({ success: false, error: "Invalid input", details: a4.errors }, { status: 400 }) : (console.error("Setup error:", a4), u.NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 }));
            }
          }
          __name(C2, "C2");
          async function D2() {
            try {
              let a3 = (0, x3.qK)();
              if (!a3) return u.NextResponse.json({ success: true, data: { setupRequired: true, reason: "Database not connected" } });
              let b5 = (0, x3.Lf)(a3), c3 = await b5.select().from(x3.qM).get();
              return u.NextResponse.json({ success: true, data: { setupRequired: !c3, adminCount: +!!c3 } });
            } catch (a3) {
              return console.error("Setup check error:", a3), u.NextResponse.json({ success: true, data: { setupRequired: true, reason: "Database error" } });
            }
          }
          __name(D2, "D2");
          let E2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/setup/route", pathname: "/api/admin/setup", filename: "route", bundlePath: "app/api/admin/setup/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/setup/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: F2, workUnitAsyncStorage: G3, serverHooks: H3 } = E2;
          function I3() {
