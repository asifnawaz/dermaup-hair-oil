...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/orders/[id]/resend-email/route.ts
// Project-relative source path: app/api/admin/orders/[id]/resend-email/route.ts
// Module ID: 37964
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 8c4d79c14ba52f6641730284528b3ece7ff6b620aa30c20159d6528d258a75d2
...
((a2, b4, c2) => {
          ...
          let B2 = v2.Ik({ emailType: v2.k5(["order_confirmation", "payment_verified", "order_shipped", "delivery_reminder", "post_delivery"]) });
          async function C2(a3, { params: b5 }) {
            ...
            try {
              let { id: d2 } = await b5, e2 = await a3.json(), { emailType: f2 } = B2.parse(e2), g3 = (0, y3.qK)();
              if (!g3) return u.NextResponse.json({ success: false, error: "Database not available" }, { status: 500 });
              let h2 = (0, y3.Lf)(g3), i2 = await h2.select().from(y3.Ww).where((0, x3.eq)(y3.Ww.id, d2)).get();
              if (!i2) return u.NextResponse.json({ success: false, error: "Order not found" }, { status: 404 });
              let j4 = (0, y3.$C)("email"), k4 = (0, A2.j)(f2, i2), l2 = { order_confirmation: y3.DT.ORDER_CONFIRMATION, payment_verified: y3.DT.PAYMENT_VERIFIED, order_shipped: y3.DT.ORDER_SHIPPED, delivery_reminder: y3.DT.DELIVERY_REMINDER, post_delivery: y3.DT.POST_DELIVERY };
              await h2.insert(y3.Fb).values({ id: j4, orderId: d2, emailType: l2[f2], recipientEmail: i2.customerEmail, subject: k4, status: y3.wu.PENDING });
              let m3 = await (0, A2.b)(f2, i2);
              return m3.success ? (await h2.update(y3.Fb).set({ status: y3.wu.SENT, sentAt: (/* @__PURE__ */ new Date()).toISOString() }).where((0, x3.eq)(y3.Fb.id, j4)), await h2.insert(y3.Y4).values({ id: (0, y3.$C)("act"), orderId: d2, action: y3.LM.EMAIL_SENT, details: JSON.stringify({ emailType: f2, resent: true }), performedBy: c3.id }), u.NextResponse.json({ success: true, data: { messageId: m3.messageId } })) : (await h2.update(y3.Fb).set({ status: y3.wu.FAILED, errorMessage: m3.error }).where((0, x3.eq)(y3.Fb.id, j4)), u.NextResponse.json({ success: false, error: m3.error || "Failed to send email" }, { status: 500 }));
            } catch (a4) {
              return a4 instanceof w3.G ? u.NextResponse.json({ success: false, error: "Invalid input", details: a4.errors }, { status: 400 }) : (console.error("Resend email error:", a4), u.NextResponse.json({ success: false, error: "Failed to resend email" }, { status: 500 }));
            }
          }
          __name(C2, "C2");
          let D2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/orders/[id]/resend-email/route", pathname: "/api/admin/orders/[id]/resend-email", filename: "route", bundlePath: "app/api/admin/orders/[id]/resend-email/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/orders/[id]/resend-email/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: E2, workUnitAsyncStorage: F2, serverHooks: G3 } = D2;
          function H3() {
          ...
          async function I3(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/orders/[id]/resend-email/route";
            e2 === "/index" && (e2 = "/");
