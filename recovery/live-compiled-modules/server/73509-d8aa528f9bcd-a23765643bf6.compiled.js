...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/coupons/[id]/route.ts
// Project-relative source path: app/api/admin/coupons/[id]/route.ts
// Module ID: 73509
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: a23765643bf6afcbc60b638f07c4421c92d5a6ec7775fd6000e59018eec77542
...
((a2, b4, c2) => {
          ...
          var d = {};
          c2.r(d), c2.d(d, { DELETE: /* @__PURE__ */ __name(() => C2, "DELETE"), GET: /* @__PURE__ */ __name(() => A2, "GET"), PUT: /* @__PURE__ */ __name(() => B2, "PUT") });
          var e = c2(95736), f = c2(9117), g2 = c2(4044), h = c2(39326), i = c2(32324), j3 = c2(261), k3 = c2(54290), l = c2(85328), m2 = c2(38928), n = c2(46595), o = c2(3421), p2 = c2(17679), q3 = c2(41681), r = c2(63446), s = c2(86439), t = c2(51356), u = c2(10641), v2 = c2(48689), w3 = c2(85766), x3 = c2(91132), y3 = c2(66147), z3 = c2(5989);
          async function A2(a3, { params: b5 }) {
            let c3 = await (0, y3.K5)(a3);
            if (!(0, y3.oC)(c3)) return u.NextResponse.json({ success: false, error: "Not authenticated" }, { status: 401 });
            try {
              let { id: a4 } = await b5, c4 = (0, x3.qK)();
              if (!c4) return u.NextResponse.json({ success: false, error: "DB unavailable" }, { status: 503 });
              let d2 = (0, x3.Lf)(c4), e2 = await d2.select().from(x3.aC).where((0, v2.eq)(x3.aC.id, a4)).get();
              return e2 ? u.NextResponse.json({ success: true, data: e2 }) : u.NextResponse.json({ success: false, error: "Coupon not found" }, { status: 404 });
            } catch (a4) {
              return console.error("Coupon get error:", a4), u.NextResponse.json({ success: false, error: "Failed to fetch coupon" }, { status: 500 });
            }
          }
          __name(A2, "A2");
          async function B2(a3, { params: b5 }) {
            ...
            if (!(0, y3.oC)(c3)) return u.NextResponse.json({ success: false, error: "Not authenticated" }, { status: 401 });
            try {
              let { id: c4 } = await b5, d2 = await a3.json(), e2 = z3.Cm.safeParse(d2);
              if (!e2.success) return u.NextResponse.json({ success: false, error: e2.error.errors[0]?.message || "Validation failed" }, { status: 400 });
              let f2 = (0, x3.qK)();
              if (!f2) return u.NextResponse.json({ success: false, error: "DB unavailable" }, { status: 503 });
              let g3 = (0, x3.Lf)(f2), h2 = e2.data, i2 = await g3.select().from(x3.aC).where((0, v2.eq)(x3.aC.code, h2.code.toUpperCase())).get();
              return i2 && i2.id !== c4 ? u.NextResponse.json({ success: false, error: "Coupon code already exists" }, { status: 409 }) : (await g3.update(x3.aC).set({ code: h2.code.toUpperCase(), type: h2.type, value: h2.value, minOrder: h2.minOrder, maxUses: h2.maxUses, appliesTo: h2.appliesTo, active: h2.active, startsAt: h2.startsAt || null, expiresAt: h2.expiresAt || null, updatedAt: (0, w3.ll)`datetime('now')` }).where((0, v2.eq)(x3.aC.id, c4)), u.NextResponse.json({ success: true, data: { id: c4 } }));
            } catch (a4) {
              return console.error("Coupon update error:", a4), u.NextResponse.json({ success: false, error: "Failed to update coupon" }, { status: 500 });
            }
          }
          __name(B2, "B2");
          async function C2(a3, { params: b5 }) {
            ...
            try {
              let { id: a4 } = await b5, c4 = (0, x3.qK)();
              return c4 ? (await (0, x3.Lf)(c4).delete(x3.aC).where((0, v2.eq)(x3.aC.id, a4)), u.NextResponse.json({ success: true, data: { id: a4 } })) : u.NextResponse.json({ success: false, error: "DB unavailable" }, { status: 503 });
            } catch (a4) {
              return console.error("Coupon delete error:", a4), u.NextResponse.json({ success: false, error: "Failed to delete coupon" }, { status: 500 });
            }
          }
          __name(C2, "C2");
          let D2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/coupons/[id]/route", pathname: "/api/admin/coupons/[id]", filename: "route", bundlePath: "app/api/admin/coupons/[id]/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/coupons/[id]/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: E2, workUnitAsyncStorage: F2, serverHooks: G3 } = D2;
          function H3() {
          ...
          async function I3(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/coupons/[id]/route";
            e2 === "/index" && (e2 = "/");
