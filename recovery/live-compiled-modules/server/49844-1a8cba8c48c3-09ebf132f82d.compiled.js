...
// Deployed source path: /home/asifnawaz/git/dermaup-hair-oil/app/api/admin/auth/login/route.ts
// Project-relative source path: app/api/admin/auth/login/route.ts
// Module ID: 49844
// Deployment location(s): dist/worker.js
// Captured factory SHA-256: 09ebf132f82dc8872030fbc3682391a7caac9dc865ee10a559f08981faeeacf3
...
((a2, b4, c2) => {
          ...
          var d = {};
          c2.r(d), c2.d(d, { POST: /* @__PURE__ */ __name(() => B2, "POST") });
          ...
          let A2 = v2.Ik({ email: v2.Yj().email(), password: v2.Yj().min(6) });
          async function B2(a3) {
            try {
              let b5 = await a3.json(), { email: c3, password: d2 } = A2.parse(b5), e2 = (0, y3.qK)();
              if (!e2) return u.NextResponse.json({ success: false, error: "Database not available" }, { status: 503 });
              let f2 = (0, y3.Lf)(e2), g3 = await f2.select().from(y3.qM).where((0, x3.eq)(y3.qM.email, c3)).get();
              if (!g3 || !await (0, z3.BE)(d2, g3.passwordHash)) return u.NextResponse.json({ success: false, error: "Invalid credentials" }, { status: 401 });
              await f2.update(y3.qM).set({ lastLogin: (/* @__PURE__ */ new Date()).toISOString() }).where((0, x3.eq)(y3.qM.id, g3.id));
              let h2 = await (0, z3.Ey)({ sub: g3.id, email: g3.email, name: g3.name, role: g3.role || "admin" });
              return await (0, z3.$G)(h2), u.NextResponse.json({ success: true, data: { token: h2, user: { id: g3.id, email: g3.email, name: g3.name, role: g3.role } } });
            } catch (a4) {
              return a4 instanceof w3.G ? u.NextResponse.json({ success: false, error: "Invalid input", details: a4.errors }, { status: 400 }) : (console.error("Login error:", a4), u.NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 }));
            }
          }
          __name(B2, "B2");
          let C2 = new e.AppRouteRouteModule({ definition: { kind: f.RouteKind.APP_ROUTE, page: "/api/admin/auth/login/route", pathname: "/api/admin/auth/login", filename: "route", bundlePath: "app/api/admin/auth/login/route" }, distDir: ".next", relativeProjectDir: "", resolvedPagePath: "/home/asifnawaz/git/dermaup-hair-oil/app/api/admin/auth/login/route.ts", nextConfigOutput: "standalone", userland: d }), { workAsyncStorage: D2, workUnitAsyncStorage: E2, serverHooks: F2 } = C2;
          function G3() {
          ...
          async function H3(a3, b5, c3) {
            var d2;
            let e2 = "/api/admin/auth/login/route";
            e2 === "/index" && (e2 = "/");
